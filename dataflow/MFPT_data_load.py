...
class MFPTBearing(object):
    ...
    def normal_set_prepare_MFPT(self, sample_num, sample_len):
        ...
        signals = []
        for i in range(2):      # to match six different working conditions, we have 3 normal bearings, sampling twice
            for file in files:
        ...
        data, labels = np.array(data), np.array(labels)
        os.chdir(self.path_cache)
        np.savez("MFPT_1D_Normal_Bearing_DataSet.npz", data=data, labels=labels)
        os.chdir(self.path)
    ...
    def normal_set_read_MFPT(self, few_num=None):

        dataset = np.load(self.path_cache + "\\MFPT_1D_Normal_Bearing_DataSet.npz")
        data = dataset["data"]
    ...
    def imbalance_case(self, normal_num=None, faulty_num=None):

        faulty_data, faulty_labels = self.working_condition_transferring(few_num=faulty_num)
        normal_data, normal_labels = self.normal_set_read_MFPT(few_num=normal_num)
