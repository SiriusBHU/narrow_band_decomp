...
class SoutheastBearingGear(object):
    ...
    def normal_set_prepare_SU(self, sample_num, sample_len):
        ...
        os.chdir(self.path_cache)
        np.savez("SU_1D_Healthy_Bearing_DataSet.npz", data=data_bearing, labels=labels_bearing)
        np.savez("SU_1D_Healthy_Gear_DataSet.npz", data=data_gear, labels=labels_gear)
        os.chdir(self.path)
    ...
    def normal_set_read_SU(self, data_need="bearing", few_num=None, chs=None):

        if data_need == "bearing":
            dataset = np.load(self.path_cache + "\\SU_1D_Healthy_Bearing_DataSet.npz")
            data = dataset["data"]
            labels = dataset["labels"]
        elif data_need == "gear":
            dataset = np.load(self.path_cache + "\\SU_1D_Healthy_Gear_DataSet.npz")
            data = dataset["data"]
            labels = dataset["labels"]
        else:
            raise ValueError("mabi name is wrong...")
    ...
    def imbalance_case_wc(self, data_need="bearing", normal_num=None, faulty_num=None, chs=None):

        if data_need == "bearing":
            faulty_data, faulty_labels = self.bearing_working_condition_transferring(few_num=faulty_num, chs=chs)
        elif data_need == "gear":
            faulty_data, faulty_labels = self.gear_working_condition_transferring(few_num=faulty_num, chs=chs)
        else:
            raise ValueError("mabi wrong name..")

        normal_data, normal_labels = self.normal_set_read_SU(data_need=data_need, few_num=normal_num, chs=chs)
