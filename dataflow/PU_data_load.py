...
class PaderbornBearing(object):
    ...
    def normal_set_prepare_PU(self, sample_num, sample_len):
        ...
        for k, file in enumerate(filenames):
            ...
            data.append(np.array(samples).reshape(4, 20 * sample_num, 1, sample_len))
            sample_labels = np.ones((80 * sample_num, 1)) * 0
            sample_labels = ut.onehot_encoding(sample_labels, 3)
            sample_labels = sample_labels.reshape(4, 20 * sample_num, -1)
            labels.append(sample_labels)
        ...
        data, labels = np.array(data), np.array(labels)
        os.chdir(self.path_cache)
        np.savez("PU_1D_normal_signal.npz", data=data, labels=labels)
        os.chdir(self.path)
    ...
    def normal_set_read_PU(self, few_num=None):

        dataset = np.load(self.path_cache + "\\PU_1D_normal_signal.npz")
        data = dataset["data"]
        ...
        data = data.transpose(1, 0, 2, 3, 4).reshape(wcs, cases * num, channels, length)
        labels = labels.transpose(1, 0, 2, 3).reshape(wcs, cases * num, -1)

        return data, labels

    def imbalance_case_ar(self, normal_num=None, faulty_num=None):

        if not isinstance(faulty_num, int):
            faulty_num = int(faulty_num)
        if not isinstance(normal_num, int):
            normal_num = int(normal_num)

        faulty_data, faulty_labels = self.artificial_real_transferring(few_num=faulty_num)
        faulty_data = faulty_data[1].reshape(17, 4, faulty_num, 1, -1)\
                                    .transpose(1, 0, 2, 3, 4)\
                                    .reshape(4, 17 * faulty_num, 1, -1)
        faulty_labels = faulty_labels[1].reshape(17, 4, faulty_num, -1)\
                                        .transpose(1, 0, 2, 3)\
                                        .reshape(4, 17 * faulty_num, -1)

        normal_data, normal_labels = self.normal_set_read_PU(few_num=normal_num)
